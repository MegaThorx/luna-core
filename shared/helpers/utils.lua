			...
			elseif type(v) == "table" then
				...
				if verbose and count > 0 and (depth == nil or depth > 0) then
					...
					for key,value in pairs(v) do
						...
						local keyString, keyTable = Utils.Dump(newModifiers,key)
						local valueString, valueTable = Utils.Dump(newModifiers,value)
...
Utils.GetElementSpeed = function(theElement, unit)
